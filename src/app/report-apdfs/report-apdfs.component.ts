...
@Component({
  selector: 'app-report-apdfs',
  templateUrl: './report-apdfs.component.html',
  styleUrls: ['./report-apdfs.component.css'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class ReportApdfsComponent {
  ...
  ngOnInit() {
    this.apiService.getVariable().subscribe(
      (response) => {
  ...
  sendReportName(reportName: string) {
    ...
    this.apiService.sendReportNameToBackend(reportName).subscribe(
      response => {
  ...
  submitGenerateReports() {
    ...
    // Llamar al servicio para enviar los datos al backend Flask
    this.apiService.generateReports(reportData).subscribe(
      (response) => {
